...
- [x] **MCP Server (Layer 3d)** — resolved, and reshaped: the original plan (`explain_grammar`/`get_vocab_drill` tools wrapping session skills) didn't hold up — `explain_grammar` was dropped from scope in Layer 2a (`grade_exercises` absorbed its only use, see Backlog in `docs/grammar.md`), and a vocab drill skill doesn't exist yet (Layer 3a is unstarted). Rebuilt instead as a read-only server over the `memory/` storage layer plus `lang/maps/` reference data — progress stats, session history, vocab flags, writing-session export, error taxonomy, grammar topics. No LLM calls, no writes, so it doesn't depend on either gap closing first. See `docs/CHECKLIST.md` Layer 3d.
- [x] **CEFR estimator + Level Progression Tracking (Layer 2c, merged with the former Layer 3b)** — resolved, and merged: these turned out to be two renderings of the same mastery data, not independent features (a single-row-per-user `user_profiles.level`, not a `user_levels` history table, is all that's ever existed — see `docs/memory.md`). Level-up is a threshold crossing on a mastery ratio (`get_module_mastery`: topics mastered ≥ `GRAMMAR_MASTERY_THRESHOLD`, texts written, weak/strong tags), not a blended weighting of text-level trend + error frequency + exercise scores — avoids inventing a second fuzzy blending rule on top of a threshold that already exists. The per-session `text_level_estimate` trend (`get_level_trend`) is kept as a separate, informational signal shown alongside the mastery bar, not folded into the gate — a text-level trend that's ahead of or behind structured mastery is itself a useful, demoable divergence, not noise to average away. Also resolved: no fixed "N texts" or "N words" threshold sourced from external research — checked, and no such number is published for either metric (see `docs/CHECKLIST.md` Layer 2c note); word/text counts are flavor stats only. See `docs/CHECKLIST.md` Layer 2c.
- [x] **Level as canonical source of truth** — resolved as part of the above: `user_profiles.level`/`level_source` (already `Literal["stated", "estimated", "cefr_module"]`) is the only place level lives — there is no separate `user_levels` table to reconcile against. The estimator only ever suggests; it does not overwrite `user_profiles.level` without user confirmation (see `docs/CHECKLIST.md` Layer 2c).
