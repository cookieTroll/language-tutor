...
|----------------------|----------------|-------------------------------------------------|
| `config.yaml`        | Ollama (local) | Ollama running, custom model built (see below)  |
...
`config.yaml`'s `llm.model` is `gemma2-9b-tutor` — a **custom** Ollama model, not a
stock one, so a plain `ollama pull gemma2-9b-tutor` will not work. It's built locally
