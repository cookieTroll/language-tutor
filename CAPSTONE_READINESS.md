...
Also worth a line in this section: the local executor model wasn't assumed, it was
tested — `qwen2.5:7b` vs `gemma2:9b` on the judge suite (e.g. 12/12 passed on
`detect_mistakes` for gemma2:9b vs 4 failures for qwen2.5:7b). qwen 7b wasn't strong
enough for reliable structured output on this skill set, so gemma2:9b became the
default local executor (`config.test.yaml`). Small, concrete evidence for "how do you
know the prompts work" beyond the two-LLM judge design itself.

One honest limitation worth a sentence in this section: judge verdicts (gemma2:9b)
are written per run (`tests/judge/results/*.json`) but nothing yet compares runs to
each other — the LLM-as-judge tier's own reliability (verdict variance run-to-run)
hasn't been quantified, only manually spot-checked once. Naming this candidly
("judge verdict stability across repeated runs isn't yet quantified") is stronger
than silence — same honesty the "Honest scope" section already asks for elsewhere,
and it preempts the obvious follow-up question ("how do you know the judge itself
is stable?"). Not a pre-submission code task: a variance script (run each judge
n=5 on a fixed commit, measure per-case verdict agreement) and a separate
regression-diff tool (fresh run vs. a committed baseline) are tracked as
post-submission engineering follow-ups in `docs/CHECKLIST.md`'s Engineering
Tooling section.
